/**
 * COMBAT UTILITIES v1.0
 * Handles initiative rolls, encounter start/end logic.
 * Compatible with D&D 5e rules.
 */
import { LootEngine } from '../services/LootEngine.js';

/**
 * Roll a d20 + modifier for initiative.
 * @param {number} dexMod - DEX modifier of the combatant
...
/**
 * Get DEX modifier from raw DEX score (D&D 5e formula).
 * @param {number} dex
 * @returns {number}
 */
...
export const combat = {
    /**
     * Rolls initiative for all tokens and returns them sorted descending.
     * Each token needs at minimum: { id, name, type, stats?: { dex } }
     * Returns an array of initiative order objects compatible with the store.
     * @param {Array} tokens - Array of token objects from TokenEngine
     * @returns {Array} sorted initiative order with rolled values
     */
    startCombat(tokens) {
    ...
    /**
     * Advance initiative to the next combatant.
     * @param {Array} order - current initiative order
     * @param {number} currentIndex - current index in order
    ...
    /**
     * End combat and return final state.
    ...
    /**
     * Generate an encounter report including XP, loot, and performance shares.
     */
