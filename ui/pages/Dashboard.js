...
import { PartyStatusHUD } from '../components/PartyStatusHUD.js';
import { ChatBox } from '../components/ChatBox.js';
import { render } from 'preact';

const MODULE_MAP = {
    'campaign': { path: () => import('../components/CampaignManager.js'), cls: 'CampaignManager' },
    'dmtable': { path: () => import('../components/DMTable.js'), cls: 'DMTable' },
    ...
    'combat': { path: () => import('../components/combat/CombatTrackerV19.js'), cls: 'CombatTrackerV19' },
    ...
    'builder': { path: () => import('../components/PlayerForm.js'), cls: 'PlayerForm' },
    'herohub': { path: () => import('../components/HeroHub.js'), cls: 'HeroHub' },
    'herosheet': { path: () => import('../components/hero/HeroSheetV19.js'), cls: 'HeroSheetV19' },
    ...
    'spellbook': { path: () => import('../components/SpellBook.js'), cls: 'SpellBook' },
        ...
        async function loadModule() {
            ...
            try {
                ...
                if (typeof Cls === 'function' && !Cls.prototype?.mount) {
                    ...
                    render(html`<${Cls} store=${store} />`, containerRef.current);
                } else {
                    // Componente Legacy
    ...
    return html`
        <div class="legacy-sheet-container animate-fadeIn">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
                <div class="card glass-accent p-4 rounded-xl text-center border-b-4 border-[${combatColor}]">
                    <div class="text-[0.65rem] text-slate-400 uppercase font-bold">Status da Sessão</div>
                    <div class="text-[1.1rem] font-extrabold text-[${combatColor}] mt-1 font-cinzel drop-shadow-[0_0_10px_${combatColor}]">${activeCombat ? '⚔️ EM COMBATE' : '📜 EXPLORAÇÃO'}</div>
                </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-10">
                ${[
                    ...
                    <button class="btn btn-premium p-4 text-[0.8rem] flex flex-col items-center justify-center gap-2 h-auto" onClick=${() => navigate(tool.id)}>
                ...
                <div class="lg:col-span-2 flex flex-col gap-8">
                    <div class="card glass-accent p-6 border-l-4 border-accent shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
                        ...
                        <div class="font-cinzel text-[0.9rem] italic leading-relaxed text-slate-300 min-h-[40px] p-4 bg-black/30 rounded-lg border border-white/5">
                            ${oracleHook || html`<span class="opacity-40 text-slate-400">Clique em "Inspirar" para gerar um gancho narrativo...</span>`}
                        </div>
                    </div>
                </div>
        ...
        <div class="w-full h-full relative">
            <div class="w-full h-full overflow-y-auto custom-scrollbar">
                ${activeTab === 'dashboard' || !activeTab 
                    ? html`<${HomePage} />` 
                    : html`<${LegacyComponentMount} tab=${activeTab} store=${TOME.store} />`
                }
            </div>
