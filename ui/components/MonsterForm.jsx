            ...
            <div class="page max-w-[1400px] mx-auto animate-fadeIn">
                <div class="flex justify-between items-center bg-black/30 border border-white/5 rounded-2xl p-6 mb-8 shadow-xl">
                    ...
                    <div class="flex gap-3 bg-black/40 p-1.5 rounded-xl border border-white/5">
                        <button class="btn ${this._view === 'library' ? 'btn-primary shadow-[0_0_15px_rgba(197,160,89,0.3)]' : 'btn-ghost'} px-4 py-2 rounded-lg font-bold text-sm transition-all" data-action="setView" data-mode="library">BIBLIOTECA</button>
                        <button class="btn ${this._view === 'creator' ? 'btn-primary shadow-[0_0_15px_rgba(197,160,89,0.3)]' : 'btn-ghost'} px-4 py-2 rounded-lg font-bold text-sm transition-all" data-action="setView" data-mode="creator">CRIAR AMEAÇA</button>
                        <button class="btn bg-blue-900/40 text-blue-300 border border-blue-500/30 hover:bg-blue-800/60 hover:text-white px-4 py-2 rounded-lg font-bold text-sm transition-all" data-action="openImporter">
                ...
                <div id="monster-importer" class="fixed inset-0 bg-black/85 z-[99999] hidden items-center justify-center backdrop-blur-md p-4">
                    <div class="card glass-accent w-full max-w-3xl p-8 border border-accent/30 rounded-2xl bg-black/95 shadow-[0_0_40px_rgba(197,160,89,0.15)] relative">
                        ...
                        <textarea id="monster-import-text" class="w-full h-[350px] mb-6 bg-black/60 text-slate-300 border border-accent/20 rounded-xl p-4 font-mono text-sm outline-none focus:border-accent focus:shadow-[inset_0_0_15px_rgba(197,160,89,0.1)] relative z-10 scrollbar-thin scrollbar-thumb-accent/30" placeholder="Ex: Owlbear / Large monstrosity, unaligned / Armor Class 13 / Hit Points 59..."></textarea>
                        
                        <div class="flex gap-4 relative z-10">
                            <button class="btn btn-ghost flex-1 py-3 text-sm font-bold rounded-xl" data-action="closeImporter">Cancelar</button>
                            <button class="btn btn-primary flex-1 py-3 text-sm font-bold font-cinzel tracking-wider rounded-xl shadow-[0_0_15px_rgba(197,160,89,0.3)]" data-action="processMonsterImport">
                                Processar & Criar
                            </button>
                        </div>
                    </div>
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="flex gap-3 overflow-x-auto pb-4 border-b border-accent/20 scrollbar-thin scrollbar-thumb-accent/30">
                    ${crGroups.map(cr => html`
                        <button class="whitespace-nowrap px-4 py-2 rounded-lg font-cinzel text-xs tracking-widest uppercase font-bold border transition-all ${this._selectedCR === cr ? 'bg-accent/20 border-accent text-accent shadow-[0_0_10px_rgba(197,160,89,0.3)]' : 'bg-transparent border-white/10 text-slate-400 hover:border-accent/50 hover:text-slate-200'} ${cr === 'BOSS' ? '!border-red-500/50 !text-red-400 hover:!bg-red-900/30' : ''}"
                ...
                <div class="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-5 max-h-[70vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-accent/30">
                    ${list.map(m => html`
                        <div class="card glass-accent flex flex-col p-0 overflow-hidden border border-white/5 hover:border-accent/50 hover:-translate-y-1 transition-all hover:shadow-[0_10px_20px_rgba(0,0,0,0.8),_0_0_15px_rgba(197,160,89,0.2)]">
    ...
    _renderCreator() {
        ...
        return html`
            <div class="card glass-accent max-w-3xl mx-auto p-8 border border-accent/30 rounded-2xl bg-black/60 shadow-[0_0_30px_rgba(0,0,0,0.8)] relative overflow-hidden">
                ...
                <form id="monster-form" class="flex flex-col gap-6 relative z-10">
