...
import { renderSpellsTab } from './PlayerSpells.js';
import { renderBioInventoryTab } from './PlayerInventory.js';
import { renderCoreTab } from './PlayerAttributes.js';
 ...
 */
export class PlayerForm extends ReactiveComponent {
    constructor(opts) {
        opts.storePath = 'players';
        super(opts);
        ...
        this._importer = new HeroImporter(this);
        this._exporter = new HeroExporter(this); // Temporary form draft to prevent data loss on new hero creation
    ...
    template() {
        ...
        return html`
            <div class="page legacy-sheet-container" style="max-width:1400px; margin: 0 auto; animation: fadeIn 0.5s ease-out;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:10px;">
                        <div class="portrait-box-legacy" style="height:350px; border:var(--sheet-border-thick); border-radius:15px; position:relative; overflow:hidden; background:rgba(0,0,0,0.2); cursor:pointer;" data-action="triggerPortrait">
                            <div id="portrait-preview" style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; overflow:hidden;">
                                ${this._portraitData ? html`<img src="${this._portraitData}" style="width:100%; height:100%; object-fit:cover; transform: scale(${this._portraitSettings.scale || 1}) translate(${this._portraitSettings.x || 0}px, ${this._portraitSettings.y || 0}px); transition: transform 0.1s ease-out;" />` : '<i class="fa-solid fa-user-shield fa-4x" style="color:rgba(197, 160, 89, 0.2);"></i>'}
                            </div>
                ...
                <!-- ════ TAB I: CORE ════ -->
                ${renderCoreTab(p, this)}

                <!-- ════ TAB II: BIO & INVENTORY ════ -->
                ${renderBioInventoryTab(p, this)}

                <!-- ════ TAB III: GRIMÓRIO ARCANO ════ -->
                ${renderSpellsTab(p, this)}
    ...
    _renderInventoryRows() {
        return this._inventoryRows.map((item, i) => html`
    ...
    _renderAttackRows() {
        return this._attackRows.map((atk, i) => html`
    ...
    submitForm() {
        ...
        if (playerData.deathSaves?.failures?.[2] === true && !this._fallenAnnounced) {
            this._fallenAnnounced = true;
            FXEngine.trigger('HERO_FALLEN', playerData.name || 'O Herói', this._editingId || 'hero-fallen');
        }
    ...
    onDeathFailureCheck(e, el) {
        if (el && el.checked && !this._fallenAnnounced) {
            this._fallenAnnounced = true;
            const f = this.$('#hero-form');
            const playerData = f ? this._collectFormData(f) : {};
            const heroName = playerData.name || 'O Herói';
            FXEngine.trigger('HERO_FALLEN', heroName, this._editingId || 'hero-fallen');
            Toast.show(`🥀 3 Falhas contra a Morte! Réquiem dos Bravos para ${heroName}...`, 'danger');
        }
    }

    resetForm() {
        this._fallenAnnounced = false;
    ...
    async importPDF(file) { await this._importer.importPDF(file); }





    async processImport() { await this._importer.processImport(); }

    downloadHeroJSON() { this._exporter.downloadHeroJSON(); }

    printOfficialSheet() { this._exporter.printOfficialSheet(); }
