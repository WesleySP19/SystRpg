    ...
    _renderHeroCard(p) {
        return `
            <div class="card glass-accent flex flex-col p-0 overflow-hidden transform hover:-translate-y-1 transition-all duration-300 hover:shadow-[0_20px_40px_rgba(0,0,0,0.9),_0_0_20px_rgba(197,160,89,0.15)] group">
                ...
                <div class="h-[140px] relative bg-cover bg-center border-b border-accent/20" style="background-image: linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(10,12,16,0.95) 100%), url('${p.portraitData || 'assets/parchment.png'}');">
                ...
                <div class="flex p-4 gap-3 bg-black/40">
                    <button class="btn btn-primary flex-1 text-[0.75rem] px-2 py-2 rounded-lg" data-action="viewSheet" data-id="${p.id}">
                        <i class="fa-solid fa-scroll"></i> Ficha 5e
                    </button>
                    <button class="btn btn-ghost text-[0.75rem] w-10 h-10 rounded-lg p-0 flex items-center justify-center opacity-70 hover:opacity-100" data-action="editHero" data-id="${p.id}" title="Editar (Forja)">
                        <i class="fa-solid fa-pen"></i>
                    </button>
                    <button class="btn btn-danger text-[0.75rem] w-10 h-10 rounded-lg p-0 flex items-center justify-center opacity-70 hover:opacity-100 border-red-500/20" data-action="deleteHero" data-id="${p.id}" title="Excluir">
