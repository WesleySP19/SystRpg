// Utility to draw text onto a card canvas with proper sizing, alignment, and multiline support.
/**
 * Draws a single line of text onto a canvas, respecting card dimensions, bleed, and safety margins.
 * @param {CanvasRenderingContext2D} ctx - 2D canvas context.
 * @param {string} text - Text to render (single line).
 * @param {Object} [options] - Configuration options.
 * @param {number} [options.dpi=300] - DPI used for the canvas.
 * @param {number} [options.cardWidthCm=6.75] - Total card width including bleed (cm).
 * @param {number} [options.cardHeightCm=9.29] - Total card height including bleed (cm).
 * @param {number} [options.marginCm=0.3] - Safety margin inside the card where text should stay (cm).
 * @param {string} [options.fontFamily='Arial'] - Font family.
 * @param {number} [options.fontSizePx] - Font size in pixels. If omitted, a sensible default is calculated.
 * @param {string} [options.color='#000'] - Text color.
 * @param {('center'|'left'|'right')} [options.align='center'] - Horizontal alignment.
 */
export function drawCardText(ctx, text, options = {}) {
  const {
    dpi = 300,
    cardWidthCm = 6.75,
    cardHeightCm = 9.29,
    marginCm = 0.3,
    fontFamily = 'Arial',
    fontSizePx,
    color = '#000',
    align = 'center',
  } = options;

  const cmToPx = cm => Math.round((cm / 2.54) * dpi);
  const cardW = cmToPx(cardWidthCm);
  const cardH = cmToPx(cardHeightCm);
  const marginPx = cmToPx(marginCm);

  const usableW = cardW - marginPx * 2;
  const usableH = cardH - marginPx * 2;

  const sizePx = fontSizePx ?? Math.round(usableH * 0.1);
  ctx.font = `${sizePx}px ${fontFamily}`;
  ctx.fillStyle = color;
  ctx.textBaseline = 'middle';

  let x;
  if (align === 'left') x = marginPx;
  else if (align === 'right') x = cardW - marginPx;
  else x = cardW / 2;

  ctx.textAlign = align;
  const y = cardH / 2;
  ctx.fillText(text, x, y, usableW);
}

/**
 * Draws multi‑line text inside the card area.
 * @param {CanvasRenderingContext2D} ctx - 2D canvas context.
 * @param {string|string[]} lines - Text lines (array or newline‑separated string).
 * @param {Object} [options] - Same options as drawCardText plus:
 * @param {number} [options.lineHeight=1.2] - Multiplier for line spacing.
 * @param {number} [options.startY] - Optional Y offset (px) for the first line.
 */
export function drawMultilineCardText(ctx, lines, options = {}) {
  const { lineHeight = 1.2, startY, ...rest } = options;
  const lineArray = Array.isArray(lines) ? lines : lines.split('\n');

  // Determine base font size using the same logic as drawCardText.
  const {
    dpi = 300,
    cardWidthCm = 6.75,
    cardHeightCm = 9.29,
    marginCm = 0.3,
    fontFamily = 'Arial',
    fontSizePx,
    color = '#000',
    align = 'center',
  } = rest;

  const cmToPx = cm => Math.round((cm / 2.54) * dpi);
  const cardW = cmToPx(cardWidthCm);
  const cardH = cmToPx(cardHeightCm);
  const marginPx = cmToPx(marginCm);
  const usableW = cardW - marginPx * 2;
  const usableH = cardH - marginPx * 2;

  const baseSize = fontSizePx ?? Math.round(usableH * 0.1);
  const linePx = Math.round(baseSize * lineHeight);

  const totalHeight = linePx * lineArray.length;
  const defaultStartY = (cardH - totalHeight) / 2;
  const yStart = startY ?? defaultStartY;

  lineArray.forEach((txt, i) => {
    const y = yStart + i * linePx + linePx / 2; // middle of each line slice
    ctx.save();
    ctx.translate(0, y - cardH / 2);
    drawCardText(ctx, txt, { dpi, cardWidthCm, cardHeightCm, marginCm, fontFamily, fontSizePx: baseSize, color, align });
    ctx.restore();
  });
}
