var y=Object.defineProperty;var v=(g,p,e)=>p in g?y(g,p,{enumerable:!0,configurable:!0,writable:!0,value:e}):g[p]=e;var l=(g,p,e)=>v(g,typeof p!="symbol"?p+"":p,e);import{R as $}from"./ReactiveComponent-I2rnF6vN.js";import{y as m,D as b,T as x}from"./BattleManager-2t4w_Qpj.js";import{Toast as h}from"./Toast-m0Ci56ke.js";import{m as d}from"./main-Dh89y2UZ.js";import"./Boot-B2dG6x9f.js";import"./jsxRuntime.module-BN06QUIv.js";import"./FXEngine-BD9eU4lT.js";import"https://cdn.socket.io/4.7.4/socket.io.esm.min.js";class T extends ${constructor(e){super(e);l(this,"close",()=>{this.element&&this.element.parentNode&&this.element.parentNode.parentNode&&this.element.parentNode.parentNode.removeChild(this.element.parentNode),this.unmount()});l(this,"setTier",e=>{this.selectedTier.value=e});l(this,"rollLoot",()=>{x.audio.playSFX("https://assets.mixkit.co/active_storage/sfx/1271/1271-preview.mp3");const e=b.roll("1d100").total,a=this.selectedTier.value,r=this._tables[a].find(t=>e>=t.range[0]&&e<=t.range[1]);if(r){const t=r.dice.split("*");let o=b.roll(t[0]).total;t[1]&&(o*=parseInt(t[1]));const c=this._rollArmadinhas(a);this.result.value={roll:e,total:o,coin:r.coin,items:c}}});l(this,"copyLoot",()=>{if(!this.result.value)return;const e=`💰 Saque: ${this.result.value.total} ${this.result.value.coin}`;navigator.clipboard.writeText(e),h.show("Copiado para a área de transferência!")});l(this,"clearResult",()=>{this.result.value=null});l(this,"openDistribute",()=>{this.selectedPlayers.value=(this.store.state.players||[]).map(e=>e.id),this.splitMode.value="equal",this.customAmounts.value={},this.showDistribute.value=!0});l(this,"closeDistribute",()=>{this.showDistribute.value=!1});l(this,"togglePlayerSelection",e=>{this.selectedPlayers.value.includes(e)?this.selectedPlayers.value=this.selectedPlayers.value.filter(a=>a!==e):this.selectedPlayers.value=[...this.selectedPlayers.value,e],this.splitMode.value="equal"});l(this,"updateCustomAmount",(e,a)=>{this.splitMode.value="custom",this.customAmounts.value={...this.customAmounts.value,[e]:parseInt(a)||0}});l(this,"confirmDistribution",()=>{const e=this.result.value,a=this.selectedPlayers.value;if(!e||a.length===0)return;const s=e.coin.split(",")[0].trim().toLowerCase();let r=0;x.store.update(o=>{o.players.forEach(i=>{if(a.includes(i.id)){let n=0;this.splitMode.value==="equal"?n=Math.floor(e.total/a.length):n=this.customAmounts.value[i.id]||0,r+=n,i.currency||(i.currency={pp:0,gp:0,ep:0,sp:0,cp:0});const u=parseInt(i.currency[s])||0;i.currency[s]=u+n}});const c=o.players.filter(i=>a.includes(i.id)).map(i=>i.name).join(", ");o.journalEntries||(o.journalEntries=[]),o.journalEntries.push({id:Date.now(),timestamp:Date.now(),type:"loot",title:"Tesouro Distribuído",content:`O saque de ${e.total} ${s.toUpperCase()} foi dividido entre: ${c}. Foram distribuídos um total de ${r} ${s.toUpperCase()}.`})});const t=e.total-r;h.show(`💰 ${r} ${s.toUpperCase()} distribuídos para ${a.length} heróis!`),t>0?h.show(`Sobrou ${t} ${s.toUpperCase()} no baú.`,"info"):t<0&&h.show(`Aviso: Foram distribuídos ${Math.abs(t)} moedas a mais do que existia no baú.`,"warning"),this.showDistribute.value=!1,this.result.value=null});l(this,"distributeItems",()=>{const e=this.result.value,a=this.selectedPlayers.value;!e||!e.items||e.items.length===0||a.length===0||(x.store.update(s=>{s.players.forEach(t=>{if(a.includes(t.id)){t.inventory||(t.inventory=[]);const o=e.items.map(c=>({...c}));t.inventory.push(...o)}});const r=s.players.filter(t=>a.includes(t.id)).map(t=>t.name).join(", ");s.journalEntries||(s.journalEntries=[]),s.journalEntries.push({id:Date.now(),timestamp:Date.now(),type:"loot",title:"Armadinhas Distribuídas",content:`Armadinhas (${e.items.map(t=>t.name).join(", ")}) foram entregues a: ${r}.`})}),h.show(`⚔️ Armadinhas distribuídas para ${a.length} heróis!`))});this.selectedTier=m("0-4"),this.result=m(null),this.showDistribute=m(!1),this.selectedPlayers=m([]),this.splitMode=m("equal"),this.customAmounts=m({}),this._armadinhas={"0-4":[{name:"Dagger",damage:"1d4",type:"piercing"},{name:"Club",damage:"1d4",type:"bludgeoning"},{name:"Shortbow",damage:"1d6",type:"piercing"}],"5-10":[{name:"Short Sword",damage:"1d6",type:"piercing"},{name:"Handaxe",damage:"1d6",type:"slashing"},{name:"Light Crossbow",damage:"1d8",type:"piercing"}],"11-16":[{name:"Longsword",damage:"1d8",type:"slashing"},{name:"Warhammer",damage:"1d8",type:"bludgeoning"},{name:"Battleaxe",damage:"1d8",type:"slashing"}],"17+":[{name:"Greatsword",damage:"2d6",type:"slashing"},{name:"Maul",damage:"2d6",type:"bludgeoning"},{name:"Heavy Crossbow",damage:"1d10",type:"piercing"}]},this._tables={"0-4":[{range:[1,30],dice:"5d6",coin:"cp"},{range:[31,60],dice:"4d4",coin:"sp"},{range:[61,70],dice:"3d6",coin:"ep"},{range:[71,95],dice:"3d6",coin:"gp"},{range:[96,100],dice:"1d6",coin:"pp"}],"5-10":[{range:[1,30],dice:"4d6*10",coin:"cp"},{range:[31,60],dice:"3d6*10",coin:"sp"},{range:[61,70],dice:"3d6*10",coin:"ep"},{range:[71,95],dice:"4d10*10",coin:"gp"},{range:[96,100],dice:"2d6*10",coin:"gp"}],"11-16":[{range:[1,20],dice:"4d6*100",coin:"sp"},{range:[21,35],dice:"1d6*100",coin:"ep"},{range:[36,75],dice:"2d10*100",coin:"gp"},{range:[76,100],dice:"2d10*100",coin:"gp"}],"17+":[{range:[1,15],dice:"2d10*1000",coin:"ep"},{range:[16,55],dice:"1d6*1000",coin:"gp"},{range:[56,100],dice:"1d6*1000",coin:"gp"}]}}_getSuggestedTier(e){if(!(e!=null&&e.length))return"0-4";const a=Math.max(...e.map(s=>parseInt(s.cr)||0));return a<=4?"0-4":a<=10?"5-10":a<=16?"11-16":"17+"}_rollArmadinhas(e){const a=this._armadinhas[e]||[];if(!a.length)return[];const s=b.roll("1d2").total,r=[];for(let t=0;t<s;t++){const o=Math.floor(Math.random()*a.length);r.push(a[o])}return r}template(){const{monsters:e,players:a}=this.store.state,s=this._getSuggestedTier(e),r=this.selectedTier.value,t=this.result.value,o=this.showDistribute.value,c=this.selectedPlayers.value;return d`
        ...
        <div class="card glass-accent animate-scaleIn" style="max-width:900px; width:100%; padding:30px; border:2px solid var(--accent); max-height:90vh; overflow-y:auto; background:rgba(15,12,16,0.95); position:relative;">
                ...
                <div style="display:flex; flex-direction:column; gap:25px;">
                    <div class="card glass-accent" style="padding:30px; background:rgba(197,160,89,0.02); border:1px solid rgba(197,160,89,0.25); border-radius:16px; box-shadow:0 10px 30px rgba(0,0,0,0.6);">
                        ...
                        <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:12px; margin-bottom:25px;">
                            ${Object.keys(this._tables).map(i=>{const n=r===i;return d`
                        ...
                        <div class="card glass-accent" style="padding:35px; border:2px solid var(--accent); border-radius:16px; text-align:center; animation: scaleIn 0.35s cubic-bezier(0.34, 1.56, 0.64, 1); box-shadow:0 0 25px rgba(197,160,89,0.25);">
                            ...
                            <div style="margin-top:10px; font-size:0.9rem; color:var(--text-main);">
                                ...
                                <ul style="list-style:none; padding:0; margin-top:5px;">
                                    ${t.items.map(i=>d`<li>⚔️ ${i.name} (${i.damage} ${i.type})</li>`)}
                                </ul>
                            </div>`:""}
                ...
                <div class="card glass-accent" style="padding:0; overflow:hidden; border-radius:16px; border:1px solid rgba(197,160,89,0.15);">
                    ...
                    <div style="padding:20px;">
                        <table style="width:100%; font-size:0.8rem; border-collapse:collapse;">
                            ...
                            <tbody>
                                ${this._tables[r].map(i=>d`
                                    <tr class="tome-hover-row" style="border-bottom:1px solid rgba(255,255,255,0.04);">
                                        <td style="padding:12px 5px; font-weight:800; color:var(--accent);">${i.range[0].toString().padStart(2,"0")}-${i.range[1].toString().padStart(2,"0")}</td>
                                        <td style="padding:12px 5px; text-align:right; font-weight:600; color:var(--text-main);">${i.dice.replace("*","x")} <span style="color:var(--accent); font-weight:800;">${i.coin.toUpperCase()}</span></td>
                                    </tr>
                                `)}
                            </tbody>
                        </table>
            ...
            ${o?d`
                <div class="modal-overlay animate-fadeIn" style="position:fixed; inset:0; background:rgba(0,0,0,0.85); backdrop-filter:blur(5px); z-index:2000; display:flex; align-items:center; justify-content:center; padding:20px;" onClick=${this.closeDistribute}>
                    <div class="card glass-accent animate-scaleIn" style="max-width:480px; width:100%; padding:30px; border:2px solid var(--accent); border-radius:16px; box-shadow:0 20px 50px rgba(0,0,0,0.9);" onClick=${i=>i.stopPropagation()}>
                        ...
                        <div style="display:flex; flex-direction:column; gap:8px; margin-bottom:25px; max-height:260px; overflow-y:auto; padding-right:5px;">
                            ${(a||[]).map(i=>{const n=c.includes(i.id);let u=0;return this.splitMode.value==="equal"?u=c.length?Math.floor(t.total/c.length):0:u=this.customAmounts.value[i.id]||0,d`
                                    <label style="display:flex; align-items:center; gap:12px; padding:12px 15px; background:${n?"rgba(197,160,89,0.08)":"rgba(255,255,255,0.02)"}; border-radius:10px; border:1px solid ${n?"var(--accent)":"rgba(255,255,255,0.06)"}; transition:all 0.2s;">
                                        <input type="checkbox" style="width:20px; height:20px; accent-color:var(--accent); cursor:pointer;" 
                                            checked=${n}
                                            onChange=${()=>this.togglePlayerSelection(i.id)} />
                                        <div style="flex:1;">
                                            <div style="font-weight:800; font-size:0.95rem; color:#fff;">${i.name}</div>
                                            <div style="font-size:0.7rem; color:var(--text-dim); text-transform:uppercase;">${i.class||"Aventureiro"}</div>
                                        </div>
                                        ${n?d`
                                            <div style="display:flex; align-items:center; gap:5px;">
                                                <input type="number" 
                                                    ...
                                                    value=${u}
                                                    onInput=${f=>this.updateCustomAmount(i.id,f.target.value)}
                                                />
